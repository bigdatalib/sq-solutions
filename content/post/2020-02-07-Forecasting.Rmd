...
Accuracy is a relative metric what we are interested here is to understand if we get fairly consistant accuracy between training and test set. Overall 3% for MAPE is fairly good
...
Another way to assess the quality of the forecast to look at how much of variation in the forcast result can be explained by the model vs. mean of the data set.
...
We can use focasting models to simulate values.
...
+ No prefect collinearity - one variable can not be linear combination of others
+ Independent variable should not have zero variance
+ error term is a "normal distribution" with mu=0
+ both independent and dependet variables draw from the population in a random sample
...
While this method can seem counter intutive and containing more steps but it provide a great deal of flexibility around independet variables. Forexample one can inlude additional regressors such as price in the model. Create sensitivity scenarios around some of the variables such as trend which will make the model good for both forcasting as well as explaining dynamics.  
...
+ **Document and Justify** - Formalizing rules and assumptions and systematic approach, Formal documentation and leniage allows for better forecast quality
...
+ **Average of other forecasters** - If available and sufficently differring forecasts can be used to generate a new forecast. We can remove extreme cases and average number of suffeciently different forcasts to come up with a new forecast. Fair market value curves and forward curves are best estimate of value of an asset at any given time. Therefore, can be used to estimate the future value of an asset. 

+ **Forecasting by Analogy** - We can use analogy of similar situation and similar context to come up with forecasts. This relies on studying historcial market dynamics in the same market and cross market. Understand if similar market fundmentals prevail what will be the price move. 

+ **Scenario forecasting** - The best approach for forcasting the future since future is generally highly uncertain. Boundary scenarios are generally much easier to forecast. For example "market clearing mechanisims" in commodities markets. Event forecasts can also be used if key events in a market are known (e.g. infrastrucutte build). Since we can be left with large number of scenarios, it is useful to come up with likelihood of the event occuring. In this case beysian inference as well as statistical methods can be used to get the likelihood of scenarios. In developing market scenarios in depth knowledge of market dynamics is essential. Dimensionality reduction methods such as PCA/PCR are useful to reduce the degrees of freedom in the problem. e.g Forward curve moves can be classified as: Parallel shift, Steepening, and changes in seasonality.  
